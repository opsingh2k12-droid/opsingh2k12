...
export function InvoicesList({ onNew }: { onNew: () => void }) {
  const [search, setSearch] = useState("")
  const [filter, setFilter] = useState<"all" | "paid" | "unpaid" | "partial" | "overdue">("all")

  const { data, isLoading } = useQuery({
    queryKey: ["tenant-invoices"],
    queryFn: async () => (await fetch("/api/tenant/invoices")).json(),
  })
  ...
  const unpaidCount = (data?.invoices || []).filter((i: any) => i.status === "unpaid").length
  const partialCount = (data?.invoices || []).filter((i: any) => i.status === "partial").length
  const overdueCount = (data?.invoices || []).filter((i: any) => i.status === "overdue").length

  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><FileText className="w-6 h-6 text-primary" /> Sales Invoices</h1>
          <p className="text-sm text-muted-foreground mt-1">{data?.invoices?.length || 0} invoices · {formatINR(total)} total</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm"><FileDown className="w-4 h-4 mr-1.5" /> Export CSV</Button>
          <Button onClick={onNew} className="gap-1.5"><Plus className="w-4 h-4" /> New Invoice</Button>
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <div className="flex gap-1 flex-wrap">
          {([
            { id: "all", label: "All", count: data?.invoices?.length || 0 },
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/30 border-b">
                <tr>
                  <th className="text-left p-3 text-xs font-medium text-muted-foreground uppercase">Invoice #</th>
